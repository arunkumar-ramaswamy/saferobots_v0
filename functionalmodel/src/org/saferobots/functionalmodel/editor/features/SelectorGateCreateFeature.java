...
public class SelectorGateCreateFeature extends AbstractCreateFeature {

	public SelectorGateCreateFeature(IFeatureProvider fp) {
		super(fp, "Selector", "Create a selector gate");
	...
	public Object[] create(ICreateContext context) {
		...
		newgate.setType(gate_type.SELECTOR);
		Port in_port_1 = factory.createPort();
		in_port_1.setType(port_type.IN);;
		Port in_port_2 = factory.createPort();
		in_port_2.setType(port_type.IN);;
		Port out_port = factory.createPort();
		out_port.setType(port_type.OUT);
		newgate.getHas_ports().add(in_port_1);
		newgate.getHas_ports().add(in_port_2);
		newgate.getHas_ports().add(out_port);
		newgate.setName("Selector");
